...
function drawClasses() {
    ...
    for (var i = 0; i < classes.count; i++) {
        template += `<div class="col race text-center" onclick="app.controllers.characterController.classInfo('${classes[i].results[i].name}')">
                         <p>${races.results[i].name}</p>   
                     </div>`
        _characterService.getSpecificRace(races.results[i].url);
    }
...
function drawCharacterProgress() {
    ...
    var character = _characterService.Race;
    console.log("CHARACTER", character);
    var template = '';
    template += `<div class="col-2">${character.name}</div> `
